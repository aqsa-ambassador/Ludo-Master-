...
def start_new_game(mode, colors_in_play, names, is_human, dice_skin, token_shape, commentary_on, groq_key):
    ...
    st.session_state.game = {
        ...
        "first_six_done": {c: False for c in colors_in_play},
    }
...
def svg_board(size=310):
    ...
    for (r,c), items in occupied.items():
        ...
        for k, (color, idx) in enumerate(items):
            ...
            cy = (r+.5)*cell
            is_active = (
                g.get("dice") is not None
                and idx in g.get("movable", [])
                and color == current_color()
            )
            token_svg = token_shape_svg(
                shape, cx, cy, cell*.30, HEX[color], DARK_HEX[color]
            )
            if is_active:
                parts.append(
                    f'<g class="active-token">'
                    f'<circle class="active-ring" cx="{cx}" cy="{cy}" r="{cell*.39}" fill="none" stroke="#111" stroke-width="2.2"/>'
                    f'{token_svg}</g>'
                )
            else:
                parts.append(token_svg)
...
# ============================================================
    ...
    if not g["is_human"][color]:
        if g["dice"] is None:
            ...
            if dice_value == 6 and not g["first_six_done"].get(color, False):
                base = [i for i,p in enumerate(g["positions"][color]) if p == -1]
                if base:
                    apply_move(color, base[0], 6)
                    g["first_six_done"][color] = True
            ...
            dice_value = g["dice"]
            with st.spinner(f"{player_name} is choosing..."):
                time.sleep(.5)
    ...
    if g["is_human"][color]:
        ...
        if dice_value is None:
            if st.button("🎲 ROLL DICE", use_container_width=True):
                ...
                # FIRST SIX: automatically open one piece. No question.
                if dice_value == 6 and not g["first_six_done"].get(color, False):
                    base = [i for i,p in enumerate(g["positions"][color]) if p == -1]
                    if base:
                        apply_move(color, base[0], 6)
            ...
            movable = g["movable"]
            if not movable:
                st.info("No move available.")
                if st.button("➡️ NEXT TURN", use_container_width=True):
                    next_turn()
                    st.rerun()
            else:
                # No Token 1/2/3/4 labels. Active pieces blink/vibrate on the board.
                cols = st.columns(len(movable))
                symbols = {"Pin":"📍", "Disc":"●", "Diamond":"◆", "Star":"★"}
                symbol = symbols.get(st.session_state.get("token_shape","Pin"), "●")
                for col, token_idx in zip(cols, movable):
                    with col:
                        if st.button(
                            symbol,
                            key=f"move_{color}_{token_idx}_{dice_value}",
                            use_container_width=True,
                            help=f"Move piece {token_idx+1}"
                        ):
                            apply_move(color, token_idx, dice_value)
                            winner = check_winner()
                            if winner:
