...
# Which color sits in which corner of the board, matching the
# home-area layout used in make_board().
CORNERS = {
    "top_left": "red",
    "top_right": "green",
    "bottom_left": "blue",
...
def mini_dice_svg(value, size=30):
    """Small dice face (with pips) used inside a corner capsule."""
    ...
    return (
        ...
        f'<rect x="1" y="1" width="{size - 2}" '
        f'height="{size - 2}" rx="7" fill="#ffffff" '
...
def status_capsule(color, game):
    """
    A small player capsule shown at the board's corner: a
    colored circle with the player's pin, plus either the dice
    face (when it's their turn and they've rolled) or a blank
    slot. The active player's capsule gets a glowing border.
    """

    is_turn = (color == current_color())
    dice_val = game["dice"] if is_turn else None

    if is_turn:
        border = f'border:2.5px solid {COLOR[color]};'
        glow = f'box-shadow:0 0 8px 1px {COLOR[color]}AA;'
    else:
    ...
    pin = (
        ...
        f'border:1.5px solid {DARK[color]};">📍</div>'
    )

    if dice_val:
        slot = mini_dice_svg(dice_val)
    else:
        slot = (
            '<div style="width:26px;height:26px;border-radius:6px;'
            'background:#f6d9d9;flex-shrink:0;"></div>'
        )

    return (
        f'<div style="display:flex;align-items:center;gap:5px;'
        f'background:#1c1c1c;border-radius:14px;'
        f'padding:4px 6px;{border}{glow}">'
        f'{pin}{slot}</div>'
    )
...
def render_board():
    """
    Renders the SVG board together with 4 corner player-status
    capsules (pin icon + dice), all inside one components.html
    call so it behaves as a single compact widget instead of
    several separately-stacked blocks (which was pushing the
    board out of view and leaving only a tall list of controls
    on screen).
    """
    game = st.session_state.game
    colors_in_game = set(game["colors"])

    def capsule_or_blank(color):
        if color in colors_in_game:
            return status_capsule(color, game)
        return '<div style="width:64px;"></div>'

    top_row = (
        '<div style="display:flex;justify-content:space-between;'
        'padding:0 6px 6px 6px;">'
        f'{capsule_or_blank(CORNERS["top_left"])}'
        f'{capsule_or_blank(CORNERS["top_right"])}'
        '</div>'
    )
    ...
    bottom_row = (
        ...
        f'{capsule_or_blank(CORNERS["bottom_left"])}'
        f'{capsule_or_blank(CORNERS["bottom_right"])}'
        '</div>'
    )
