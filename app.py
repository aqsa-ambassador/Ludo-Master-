...
st.markdown(
    """
    <style>
    .block-container {padding-top: 0.6rem; padding-bottom: 0.4rem; max-width: 440px;}
    #MainMenu {visibility: hidden;}
    footer {visibility: hidden;}
    header {visibility: hidden; height: 0;}
    .stButton>button {padding: 0.3rem 0.5rem;}
    h1 {font-size: 1.5rem !important; margin-bottom: 0.1rem !important;}
    h3 {margin-top: 0 !important; margin-bottom: 0 !important;}
    div[data-testid="stVerticalBlock"] > div[data-testid="stVerticalBlockBorderWrapper"] {margin-bottom: 0 !important;}
    div[data-testid="stExpander"] {margin-top: 0.2rem !important;}
    div.element-container {margin-bottom: 0.15rem !important;}
...
CORNER_OF = {"red": "tl", "green": "tr", "yellow": "br", "blue": "bl"}


def pill_html(color, name, tag, is_turn):
    border = f"2px solid {HEX[color]}" if is_turn else "1px solid #3a3a3a"
    bg = f"{HEX[color]}2a" if is_turn else "#1c1c1c"
    glow = f"box-shadow:0 0 6px {HEX[color]}99;" if is_turn else ""
    arrow = "▶ " if is_turn else ""
    return f"""<div style="border:{border};{glow}border-radius:9px;background:{bg};
        padding:3px 6px;display:flex;align-items:center;gap:5px;min-width:0;">
        <div style="width:11px;height:11px;min-width:11px;border-radius:50%;background:{HEX[color]};"></div>
        <div style="font-size:0.68rem;line-height:1.05;color:#eee;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">
        {arrow}{name}<br><span style="opacity:0.6;font-size:0.6rem;">({tag})</span></div>
        </div>"""
...
def render_game_widget(g, turn_color):
    pills = {}
    for color in COLORS:
        if color in g["colors"]:
            tag = "You" if g["is_human"][color] else "Bot"
            pills[color] = pill_html(color, g["names"][color], tag, color == turn_color)
        else:
            pills[color] = empty_pill_html()

    board_svg = svg_board(size=310)
    dice_svg = svg_dice(g["dice"], st.session_state.dice_skin)
    my_tag = "You" if g["is_human"].get(turn_color, True) else "Bot"
    team_note = f" · Team {TEAM_OF[turn_color]}" if g["mode"] == "team" else ""

    return f"""
    <div style="font-family:sans-serif;">
      <div style="display:flex;justify-content:space-between;gap:8px;margin-bottom:4px;">
        <div style="flex:1;">{pills['red']}</div>
        <div style="flex:1;">{pills['green']}</div>
      </div>
      <div style="width:310px;margin:0 auto;">{board_svg}</div>
      <div style="display:flex;justify-content:space-between;gap:8px;margin-top:4px;">
        <div style="flex:1;">{pills['blue']}</div>
        <div style="flex:1;">{pills['yellow']}</div>
      </div>
      <div style="display:flex;align-items:center;justify-content:space-between;
          margin-top:8px;padding:4px 8px;border-radius:9px;background:#161616;">
        <div style="font-size:0.82rem;color:#eee;">
          <b>Turn:</b> <span style="color:{HEX[turn_color]};font-weight:800;">{g['names'][turn_color]}</span>
          <span style="opacity:0.65;">({my_tag}){team_note}</span>
        </div>
        <div style="width:44px;">{dice_svg}</div>
      </div>
    </div>
    """
...
# ============================================================
    ...
    if g["winner"]:
        st.markdown(
            f"<div style='background:#1c3a1c;border:1px solid #2e7d32;border-radius:8px;"
            f"padding:5px 8px;font-size:0.85rem;margin-bottom:2px;'>🏆 <b>{g['winner']}</b> wins!</div>",
            unsafe_allow_html=True,
        )

    turn_color = current_color()
    st.components.v1.html(render_game_widget(g, turn_color), height=470, scrolling=False)
